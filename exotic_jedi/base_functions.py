...
def dq_flat_replace(data_cube, dq_cube, bits_to_mask=[0, 2, 10, 11], window_size=4):
	...
	for i in tqdm(range(len(flags_time))):
		...
		if np.any(bit_array[bits_to_mask] == 1):
			...
			sci_cube[hiti, hity, hitx] = np.nanmedian(window)
...
def outliers_through_space(input_data, replace_window=4, search_window=10, poly_order=9, n_sig=20, plot=False):
	...
	if plot == True:
		median_before = np.nanmedian(input_data, axis=0)
		median_after = np.nanmedian(data_to_window, axis=0)
...
def outliers_through_time(input_data, window_size=30, n_sig=6, plot=False):
	...
	while True:
		...
		medians = np.nanmedian(new_data, axis=0)
		...
		for hiti,hity,hitx in zip(*np.where(cr_mask!=0)):
			...
			new_data[hiti, hity, hitx] = np.nanmedian(window)
...
def check_1f(im_stack, fnoise_mask, stack=True):
	...
	if stack == True:
		power_clean = np.nanmedian(powers_clean,axis=0) # average the power spectra of all the images
		power = np.nanmedian(powers,axis=0)
...
def get_stellar_spectra(data_cube, err_cube, upper_ap, lower_ap, set_to_edge = True, xarray=None, flat=None, f_mask=None, extract_method="intrapixel", shift=True, interpolate_mode="cubic", trim_spec=[3,1], high_res_factor=[0.01,0.01], trim_fit=[10,10], plot=False):
	...
	if shift == True:
		x_template = np.nanmedian(unshifted_x, axis=0)
		y_template = np.nanmedian(unshifted_y, axis=0)
...
def compare_2d_spectra(clean_spectra, unclean_spectra, wvl, time, time_units="BJD", 
	...
	Z = (clean_spectra)
	Z_norm = (clean_spectra/ np.nanmedian(clean_spectra, axis=0))

	Z_old = (unclean_spectra)
	Z_old_norm = (unclean_spectra/ np.nanmedian(unclean_spectra, axis=0))
